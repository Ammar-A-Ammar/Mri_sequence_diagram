...
if (1 == True):
    ...
    for item in RF:
        ...
        Duration = item.get('Duration')
        if Sign =="-":
            excitation, slice_selection = diagram.selective_pulse("RF", "$G_z$", duration=Duration,gradient_amplitude=-1, pulse_amplitude=Flip_A, ramp=d_ramp, center=Time)
        elif Sign =="+":
            excitation, slice_selection = diagram.selective_pulse("RF", "$G_z$", duration=Duration,gradient_amplitude=-1, pulse_amplitude=Flip_A, ramp=d_ramp, center=Time)
    ...
    for item in Multi_gradient:
        ...
        Sign = item.get('Sign')
        if (item.get('Axis') == "y"):
            multiGradient = diagram.multi_gradient("$G_y$", d_encoding, 0.5, d_ramp, center = Time)
            if Sign=="+":
                diagram.annotate("$G_y$",multiGradient.center+0.5, 0, r"$\uparrow$" )
            elif Sign=="-":
                diagram.annotate("$G_y$",multiGradient.center+0.5, 0, r"$\downarrow$" )
        elif (item.get('Axis') == "x"):
            multiGradient = diagram.multi_gradient("$G_x$", d_encoding, 0.5, d_ramp, center = Time)
            if Sign=="+":
                diagram.annotate("$G_x$",multiGradient.center+0.5, 0, r"$\uparrow$" )
            elif Sign=="-":
                diagram.annotate("$G_x$",multiGradient.center+0.5, 0, r"$\downarrow$" )
        elif (item.get('Axis') == "z"):
            multiGradient = diagram.multi_gradient("$G_z$", d_encoding, 0.5, d_ramp, center = Time)
            if Sign=="+":
                diagram.annotate("$G_z$",multiGradient.center+0.5, 0, r"$\uparrow$" )
            elif Sign=="-":
                diagram.annotate("$G_z$",multiGradient.center+0.5, 0, r"$\downarrow$" )
